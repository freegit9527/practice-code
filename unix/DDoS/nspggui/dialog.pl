...
sub dpm_isvalid {
	local($rc) = system("which dialog >/dev/null");
	chop($rc);
	return 0 if ($rc != 0);
	#&dpm_maxsize();
	return 1;
...
sub dpm_maxsize {
#	local($rc) = `dialog --print-maxsize 2>&1`;
#	$rc=~s/[\n\r ]//g;
#	$rc=~s/^MaxSize://g;
#	local(@eles) = split(/,/, $rc);
#	local($width, $height) = ($eles[1], $eles[0]);	# return width, height
	$data = `stty -a`;
...
sub dpm_msgbox {
    ...
    $message = &dpm_wordwrap($message, $width);
...
sub dpm_update_gauge {
    local ( $percent ) = @_;

    &printflush(GAUGE, "$percent\n");
}

sub dpm_update_gauge_and_message {
    local ( $message, $percent ) = @_;

    $message = &dpm_wordwrap($message, $gauge_width);
    $message =~ s/\n/\\n/g;
